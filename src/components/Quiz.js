...
const Quiz = () => {
  ...
  const generateQuestion = () => {
    ...
    if (!sentenceTemplates[templateKey]) {
      console.error(
        `Template key "${templateKey}" not found in sentenceTemplates`
      );
      return;
    }
        ...
        <div>
            ...
            {currentWord.template
              ...
              .replace(
                ...
                getArticle(
                  ...
                  currentWord.isPlural
                )
              )
              .replace(
                currentWord.isPlural ? "noun_plural" : "noun",
                `${currentWord.word} (${currentWord.gender}, ${
                  currentWord.isPlural ? "Pl." : "Sing."
                })`
              )}
          </p>
          ...
          {result !== null && (
            <p>
              {result
                ? "Правильно!"
                : `Неправильно. Правильный ответ: ${getCorrectAnswer(
                    currentWord,
                    currentAdjective
                  )}`}
            </p>
